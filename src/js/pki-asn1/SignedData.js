import { clearProps, getParametersValue, isEqualBuffer, utilConcatBuf } from "pvutils";

import AlgorithmIdentifier from "./AlgorithmIdentifier";
import { compareSchema, Constructed, fromBER, Integer, Null, OctetString, Repeated, Sequence, Set } from "./asn1";
...
import { getAlgorithmByOID, getCrypto, getEngine, getOIDByAlgorithm } from "./common";
...
export default class SignedData 
{
	...
	} = {})
	{
		...
		else // Find by SubjectKeyIdentifier
		{
			sequence = (async () => {
				try {
					const {sid} = this.signerInfos[signer];
