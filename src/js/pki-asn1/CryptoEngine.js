import { arrayBufferToString, getParametersValue, stringToArrayBuffer, utilConcatBuf } from "pvutils";

import AlgorithmIdentifier from "./AlgorithmIdentifier";
import { fromBER, Null, OctetString } from "./asn1";
...
function makePKCS12B2Key(cryptoEngine, hashAlgorithm, keyLength, password, salt, iterationCount)
{
	...
	for(let i = 0; i < passwordViewInitial.length; i += 1)
	{
	...
	for(let i = 0; i < D.byteLength; i += 1)
	...
	for(let i = 0; i <= c; i += 1)
	{
		...
		internalSequence = internalSequence.then(roundBuffer =>
		{
			...
			for(let j = 0; j < B.byteLength; j += 1)
...
export default class CryptoEngine
{
	...
	decryptEncryptedContentInfo(parameters)
	{
		...
		const {iterationCount} = pbkdf2Params;
