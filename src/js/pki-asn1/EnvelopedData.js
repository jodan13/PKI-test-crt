import { clearProps, getParametersValue, utilConcatBuf } from "pvutils";

import AlgorithmIdentifier from "./AlgorithmIdentifier";
import { compareSchema, Constructed, fromBER, Integer, Null, ObjectIdentifier, OctetString, Repeated, Sequence, Set } from "./asn1";
import Attribute from "./Attribute";
import { getAlgorithmByOID, getCrypto, getOIDByAlgorithm, getRandomValues, kdf } from "./common";
...
export default class EnvelopedData
{
	...
	{
		// region Initialize encryption parameters
		const encryptionParameters = { ...defaultEncryptionParams, ...parameters || {}};
		// endregion
	...
	encrypt(contentEncryptionAlgorithm, contentToEncrypt)
	{
		...
		function SubKeyAgreeRecipientInfo(index)
		{
			...
			currentSequence = currentSequence.then(result =>
			{
				 ...
				 */
				recipientInfo.value.recipientEncryptedKeys.encryptedKeys[0].encryptedKey = new OctetString({ valueHex: result });
				// endregion
		...
		async function SubKeyTransRecipientInfo(index)
		{
			...
			try
			{
				...
				// region RecipientEncryptedKey
				_this.recipientInfos[index].value.encryptedKey = new OctetString({valueHex: encryptedKey});
				// endregion
			}
			catch(ex)
			{
		...
		sequence = sequence.then(() =>
		{
			for(let i = 0; i < this.recipientInfos.length; i += 1)
			{
				// region Initial variables
				let currentSequence = Promise.resolve();
				// endregion
	...
	decrypt(recipientIndex, parameters)
	{
		...
		function SubKeyAgreeRecipientInfo(index)
		{
			...
			const {originator} = _this.recipientInfos[index].value;
