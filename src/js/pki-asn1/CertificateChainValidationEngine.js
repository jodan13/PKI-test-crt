...
export default class CertificateChainValidationEngine
{
	...
	async sort(passedWhenNotRevValues = false)
	{
		...
		async function buildPath(certificate)
		{
			...
			for(let i = 0; i < findIssuerResult.length; i += 1)
			{
				...
				for(let j = 0; j < buildPathResult.length; j += 1)
				{
		...
		async function findCRL(certificate)
		{
			...
			for(let i = 0; i < crls.length; i += 1)
			{
				...
				for(let j = 0; j < issuerCertificates.length; j += 1)
				{
					try
					{
		...
		async function findOCSP(certificate, issuerCertificate)
		{
			...
			for(let i = 0; i < _this.ocsps.length; i += 1)
			{
		...
		async function checkForCA(certificate, needToCheckCRL = false)
		{
			...
			if("extensions" in certificate)
			{
				for(let j = 0; j < certificate.extensions.length; j += 1)
				{
			...
			if((_this.crls.length !== 0) || (_this.ocsps.length !== 0)) // If CRLs and OCSPs are empty then we consider all certificates to be valid
			{
				for(let i = 0; i < (path.length - 1); i += 1)
				{
					...
					if(_this.crls.length !== 0)
					{
						...
						{
							for(let j = 0; j < crlResult.result.length; j += 1)
							{
		...
		for(let i = 0; i < localCerts.length; i += 1)
		{
			for(let j = 0; j < localCerts.length; j += 1)
			{
				if(i === j)
					{continue;}
		...
		for(let i = 0; i < result[shortestIndex].length; i += 1)
	...
	async verify(parameters = {})
	{
		...
		function compareDirectoryName(name, constraint)
		{
			...
			for(let i = 0; i < constraint.typesAndValues.length; i += 1)
			{
				let localResult = false;
				
				for(let j = nameStart; j < name.typesAndValues.length; j += 1)
				{
		...
		try
		{
			...
			for(let i = (this.certs.length - 2); i >= 0; i--, pathDepth++)
			{
				if("extensions" in this.certs[i])
				{
					...
					for(let j = 0; j < this.certs[i].extensions.length; j += 1)
					{
			...
			for(let i = 0; i < (this.certs.length - 1); i += 1)
			{
				...
				if((i < (this.certs.length - 2)) && (typeof policyMappings[i + 1] !== "undefined"))
				{
					for(let k = 0; k < policyMappings[i + 1].mappings.length; k++)
					{
						...
						for(let j = 0; j < certPolicies[i].certificatePolicies.length; j += 1)
						{
			...
			for(let i = 0; i < allPolicies.length; i += 1)
			{
				if(allPolicies[i] === "2.5.29.32.0")
				{
					for(let j = 0; j < explicitPolicyStart; j += 1)
			...
			for(let i = 0; i < policiesAndCerts.length; i += 1)
			{
				let found = true;
				
				for(let j = 0; j < (this.certs.length - 1); j += 1)
				{
			...
			if((authConstrPolicies.length === 1) && (authConstrPolicies[0] === "2.5.29.32.0"))
				...
				{
					for(let i = 0; i < authConstrPolicies.length; i += 1)
					{
						for(let j = 0; j < initialPolicySet.length; j += 1)
						{
			...
			for(let i = (this.certs.length - 2); i >= 0; i--, pathDepth++)
			{
				...
				for(let j = 0; j < requiredNameForms.length; j += 1)
				{
				...
				for(let j = 0; j < permittedSubtrees.length; j += 1)
				{
				...
				for(let j = 0; j < excludedSubtrees.length; j += 1)
				{
